...
int main(int argc, char **argv)
{
  ...
  double x_init;
  double y_init;
  nh.param<double>("map_width", map_width, 28);
  nh.param<double>("map_height", map_height, 28);
  nh.param<double>("map_resolution", map_resolution, 0.25);
  nh.param<double>("offset_x", x_init, 0.0);
  nh.param<double>("offset_y", y_init, 0.0);
  local_map::MapBuilder map_builder(map_width, map_height, map_resolution,x_init,y_init);
  map_builder_ptr = &map_builder;

  ros::Subscriber scanHandler = nh.subscribe<sensor_msgs::LaserScan>("/hsrb/base_scan", 10, handleLaserScan);
  map_publisher = nh.advertise<nav_msgs::OccupancyGrid>("local_map", 10, true);
