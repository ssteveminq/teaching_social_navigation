...
bool RECEIVED_PROJ_MAP_DATA = false;
...
public:
	...
	tf::TransformListener 			odom_to_world_listener;
	tf::StampedTransform 			odom_to_world_transform;

	tf::TransformListener 			map_to_world_listener;
	tf::StampedTransform 			map_to_world_transform;

	tf::TransformListener 			base_link_to_world_listener;
	...
	void rtabmap_callback(const nav_msgs::OccupancyGrid::ConstPtr &msg);
    void odom_state_callback(const nav_msgs::Odometry::ConstPtr &msg);
...
void GridMap::rtabmap_callback(const nav_msgs::OccupancyGrid::ConstPtr &msg){
	if (RECEIVED_PROJ_MAP_DATA == false){
		proj_map_grid = (*msg);
		proj_map_grid.header.stamp =  ros::Time::now	();
		proj_map_grid.header.frame_id = "map"; 
		RECEIVED_PROJ_MAP_DATA = true;
	}else{/*
	//	Update current environment
		float x_offset = msg->info.origin.position.x;
		float y_offset = msg->info.origin.position.y;
		float res = msg->info.resolution;

		int map_width = msg->info.width;
		int map_height = msg->info.height;	

		// Only update cells that are NOT human or robot occupied
		for(size_t index = 0; index < msg->data.size(); index++){
	   		int i = index % map_width;
	   		int j = floor(index / map_width);

			float x_coord_center = i*res + (res/2.0) - x_offset;
			float y_coord_center = j*res + (res/2.0) - x_offset;			

			int new_occ_type = FREE_CELL; // initialize

			// Assign all obstacle cells as free
			// Call BoundingBox Occupancy
			std::map<int, int> indices_encapuslated = bounding_box_to_world_indices(x_coord_center, y_coord_center, res, res);
			typedef std::map<int, int>::iterator it_type;
			for(it_type iterator = indices_encapuslated.begin(); iterator !=  indices_encapuslated.end(); iterator++) {
				int world_index = iterator->first; 
				int cell_type = data[world_index].cell_type;

				if (cell_type == OBSTACLE_OCCUPIED){
					 data[world_index].cell_type = FREE_CELL;
				}
			}

			//Assign back all partially free cells as obstacles
			if ((msg->data[index] == 100) || (msg->data[index] == -1)) {
				new_occ_type = OBSTACLE_OCCUPIED;
			}	

			if (new_occ_type == OBSTACLE_OCCUPIED){
				for(it_type iterator = indices_encapuslated.begin(); iterator !=  indices_encapuslated.end(); iterator++) {
					int world_index = iterator->first; 
					int cell_type = data[world_index].cell_type;

					if ((cell_type == FREE_CELL) || (cell_type == OBSTACLE_OCCUPIED)) {
						 data[world_index].cell_type = OBSTACLE_OCCUPIED;
					}
				}				
...
void GridMap::odom_state_callback(const nav_msgs::Odometry::ConstPtr &msg){
	// Transforms here
	float odom_robot_x = msg->pose.pose.position.x;
	float odom_robot_y = msg->pose.pose.position.y;	
	float odom_robot_z = msg->pose.pose.position.z;
	...
	// Do transforms here;
	try{
	  odom_to_world_listener.lookupTransform(WORLD_FRAME, frame_id, ros::Time(0), odom_to_world_transform);
    }
	...
	// Calculate Position of Base Link in world wrame
	tf::Vector3 r_vec_odom_frame(odom_robot_x, odom_robot_y, 0);
	tf::Stamped<tf::Vector3> stamped_r_vec_odom_frame(r_vec_odom_frame, ros::Time(0), frame_id);
	tf::Vector3 r_vec_world_frame;
	tf::Stamped<tf::Vector3> stamped_r_vec_world_frame(r_vec_world_frame,  ros::Time(0), WORLD_FRAME);

	odom_to_world_listener.transformVector(WORLD_FRAME, ros::Time(0), stamped_r_vec_odom_frame, WORLD_FRAME, stamped_r_vec_world_frame);

	this->robot_world_x_pos = odom_to_world_transform.getOrigin().x() + stamped_r_vec_world_frame.getX();
	this->robot_world_y_pos = odom_to_world_transform.getOrigin().y() + stamped_r_vec_world_frame.getY();
	...
	bounding_box_to_occupany(robot_world_x_pos, robot_world_y_pos, TRIKEY_SIDE_WIDTH, TRIKEY_SIDE_WIDTH, ROBOT_OCCUPIED);
...
// try{
// 	  base_link_to_world_listener.lookupTransform(WORLD_FRAME, "base_link", ros::Time(0), base_link_to_world_transform);
//     }
//     catch (tf::TransformException &ex) {
//       ROS_ERROR("%s",ex.what());
//       return;
//     }
// 	// Calculate Position of Base Link in world wrame
// 	tf::Vector3 r_vec_base_link_frame(1.0, 0.0, 0);
// 	tf::Stamped<tf::Vector3> stamped_r_vec_base_link_frame(r_vec_base_link_frame, ros::Time(0), "base_link");
// 	tf::Vector3 r_vec_world_frame;
// 	tf::Stamped<tf::Vector3> stamped_r_vec_world_frame(r_vec_world_frame,  ros::Time(0), WORLD_FRAME);

// 	base_link_to_world_listener.transformVector(WORLD_FRAME, ros::Time(0), stamped_r_vec_base_link_frame, WORLD_FRAME, stamped_r_vec_world_frame);


// 	float unit_base_link_x = stamped_r_vec_world_frame.getX();
// 	float unit_base_link_y = stamped_r_vec_world_frame.getY();

// 	std::cout << "base link x:" << unit_base_link_x << std::endl;
// 	std::cout << "base link y:" << unit_base_link_y << std::endl;	

// 	nav_info_msg.unit_base_link_x = unit_base_link_x;
// 	nav_info_msg.unit_base_link_y = unit_base_link_y;
...
int main(int argc, char **argv)
{
  ...
  GridMap gridmap;
  	...
  	gridmap.proj_map_sub =  gridmap.node.subscribe<nav_msgs::OccupancyGrid>("rtabmap/proj_map", 10, boost::bind(&GridMap::rtabmap_callback, &gridmap, _1));

	ros::Rate r(10); 
	// Subscribe to rtabmap/proj_map
	while(ros::ok()){
		if (RECEIVED_PROJ_MAP_DATA == true){
			gridmap.init_all();
			break;
		}
		std::cout <<  "waiting for /rtabmap/proj_map topc" << std::endl;
		ros::spinOnce();
		r.sleep();
	}
  ...
  gridmap.trikey_state_sub = gridmap.node.subscribe<nav_msgs::Odometry>("odom", 10, boost::bind(&GridMap::odom_state_callback, &gridmap, _1));
